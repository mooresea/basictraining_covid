...
numStaff = ceiling(0.05*numrecruits) ## assume numStaff = 5% of numrecruits
...
average.contacts =
  unlist(lapply(contacts.random,length)) +
  unlist(lapply(contacts.cocoon,length))*(14/time_max) +
  unlist(lapply(contacts.company,length))*((time_max-14)/time_max) 
